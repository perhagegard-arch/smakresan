...
function exportData(){
  ...
  const blob = new Blob([JSON.stringify({app:"pers-whiskyresa", exportedAt:new Date().toISOString(), state:st, photos:includePhotos?photos:{}}, null, 1)], {type:"application/json"});
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob);
  a.download = "pers-whiskyresa-backup-"+today()+".json";
  a.click();
...
function importData(file){
  ...
  reader.onload = () => {
    try{
      ...
      renderView();
    }catch(e){ alert("Filen gick inte att läsa – är det en Pers whiskyresa-backup?"); }
  };
...
function openGuestView(inv){
  ...
  let html = '<div class="ov-top"><span class="mono">Pers whiskyresa · Gästprovning</span></div>'+
